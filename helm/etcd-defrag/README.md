...
| etcdStorageQuotaBytes | int | `8589934592` | etcd storage quota in bytes. |
| hostNetwork | bool | `true` | Wheter to use host network or not. |
| image | object | `{"image":"ahrtr/etcd-defrag","pullPolicy":"IfNotPresent","registry":"ghcr.io","tag":""}` | Image settings. |
| image.image | string | `"ahrtr/etcd-defrag"` | Image name. |
